import { type ForwardedRef, type WheelEvent, forwardRef } from 'react';
      ...
      <MUITextField
        ...
        slotProps={{
          input: {
            ...
            startAdornment: (
              prependObject && <InputAdornment position="start" classes={{ root: '[&>*]:fill-n-7 *:text-base' }}>{prependObject}</InputAdornment>
            ),
            endAdornment: (
              appendObject && <InputAdornment position="end" classes={{ root: '[&>*]:fill-n-7 *:text-base' }}>{appendObject}</InputAdornment>
            ),
            inputProps: {
              maxLength,
              className: `py-3 px-3.5 ${!!prependObject && 'pl-0'} ${!!appendObject && 'pr-0'}`,
          ...
          inputLabel: { shrink: !!value, className: 'text-base -my-1' },
        }}
      />
    </div>
  );
